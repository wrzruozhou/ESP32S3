...
typedef struct {
    rmt_encoder_t base;                                 /**基础类，声明标准编码器接口*/
    ...
    rmt_symbol_word_t nec_ending_symbol;                /**具有RM表示的NEC结束代码*/
    int state;
...
//    rmt_ir_nec_encoder_t *nec_encoder1 = (rmt_ir_nec_encoder_t*)encoder;
    rmt_encode_state_t session_state = RMT_ENCODING_RESET;//0
    rmt_encode_state_t state = RMT_ENCODING_RESET;//0
...
static esp_err_t rmt_ir_nec_encoder_reset(rmt_encoder_t *encoder)
{
    ...
    nec_encoder->state = RMT_ENCODING_RESET;//0
