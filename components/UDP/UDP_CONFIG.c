...
void connet_display(uint8_t flag)
{
    if((flag & 0x80) == 0x80)
    {
        ...
        ltdc_show_string(0, 90, 240, 16, 16, lcd_buff, BLUE);
        sprintf(lcd_buff, "psw:%s",DEFAULT_PWD);
        ltdc_show_string(0, 110, 240, 16, 16, lcd_buff, BLUE);
        ltdc_show_string(0, 130, 200, 16, 16, "KEY0:Send data", MAGENTA);
    }
    else if ((flag & 0x04) == 0x04)
    {
        ltdc_show_string(0, 90, 240, 16, 16, "wifi connecting......", BLUE);
    }
    else if ((flag & 0x02) == 0x02)
    {
        ltdc_show_string(0, 90, 240, 16, 16, "wifi connecting fail", BLUE);
    }
    else if ((flag & 0x01) == 0x01)
    {
        ltdc_show_string(0, 150, 200, 16, 16, (char*)network_connet.ip_buf, MAGENTA);
    }
